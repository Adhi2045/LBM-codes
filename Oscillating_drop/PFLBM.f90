...
MODULE SHARE
    ...
    INTEGER,PARAMETER :: L0 = 256

    INTEGER,PARAMETER :: tf   = 1000
    INTEGER,PARAMETER :: step = tf/100
    ...
    REAL(8),PARAMETER :: R = L0/8.d0 ! original
    ...
    REAL(8) :: tmp, Ri, Fx, Fy, r_theta, theta, dx, dy, dist
...
SUBROUTINE Initialize_distributions
    ...
    DO X = 0, Nx+1  !!1, Nx
     ...
     !    Ri = DSQRT( ((X-(X0))*(X-(X0)))/(a*a) + ((Y-(Y0))*(Y-(Y0)))/(b*b) )  ! equation for ellipse.
      !   Ri = ABS(X-X0) + ABS(Y-Y0) ! equation for diamond

! Shifted coordinates from center
        dx = X - X0
        dy = Y - Y0

        ! Polar coordinates: angle from center
        theta = ATAN2(dy, dx)

        ! Star-shaped radius at angle theta
        r_theta = R * (1.0 + eps * COS(n * theta))  ! eps < 1, n = number of spikes

        ! Distance from center
        dist = DSQRT(dx*dx + dy*dy)

        ! Phase field initialization: star-shaped droplet
        C(X,Y) = 0.5d0 + 0.5d0 * TANH(2.0d0 * (r_theta - dist) / W)
...
!    CALL Macroscopic_Properties_h
    ...
    !	Fx = mu(X,Y) * DcDx(X,Y) + FpX + FmX + FbX
        Fx = mu(X,Y) * DcDx(X,Y) + FpX !+ FmX 
        Fy = mu(X,Y) * DcDy(X,Y) + FpY !+ FmY 
    !	Fy = mu(X,Y) * DcDy(X,Y) + FpY + FmY + FbY
