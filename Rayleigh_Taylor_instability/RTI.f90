...
MODULE SHARE
    ...
    INTEGER,PARAMETER :: Ny = 4*L0

    INTEGER,PARAMETER :: X0 = Nx/2
...
!    REAL(8),PARAMETER :: b = R/32.d0 ! new added
    ...
    REAL(8),PARAMETER :: gY = 0.000001d0
...
SUBROUTINE Initialize_distributions
    ...
    !    C(X,Y) = 0.5d0 - 0.5d0 * TANH(2*(R-Ri)/W)    !bubble
        Ri = Ny/2 + 0.1 * L0 * dCOS(2*pai*real(X)/L0)
        C(X,Y) = 0.5d0 + 0.5d0 * TANH(2*(real(Y)-Ri)/W)
