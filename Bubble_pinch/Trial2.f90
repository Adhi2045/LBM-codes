...
MODULE SHARE
    ...
    INTEGER,PARAMETER :: Nx = L0/2
    INTEGER,PARAMETER :: Ny = L0

    INTEGER,PARAMETER :: X0 = Nx/2 + 1
    INTEGER,PARAMETER :: Y0 = L0/16 + 1
    ...
    INTEGER,PARAMETER :: R = 8 !L0/8.d0 ! original
!    REAL(8),PARAMETER :: a = R/64.d0 ! new added
!    REAL(8),PARAMETER :: b = R/32.d0 ! new added
    
    REAL(8),PARAMETER :: Rhol  = 1d-3
    REAL(8),PARAMETER :: Rhoh  = 1.0d0
    REAL(8),PARAMETER :: dRho3 = (Rhoh - Rhol)/3
    REAL(8),PARAMETER :: dRho2 = (Rhoh - Rhol)/2 ! added for gravity or density force.

    REAL(8),PARAMETER :: Sigma = 0.001242d0
    REAL(8),PARAMETER :: gY = 4d-6
    REAL(8),PARAMETER :: pai = 3.141592653589793d0 ! added value of pai

    REAL(8),PARAMETER :: tau_H = 0.8d0 !0.8d0 !0.3d0 + 0.5d0 !(3.d0*U0*real(Nx))/Re mu was taken 0.005
    REAL(8),PARAMETER :: tau_L = 0.8d0  !0.8d0  !0.3d0 + 0.5d0 !(3.d0*U0*real(Nx))/Re !10.d0*(3.d0*U0*real(Nx))/Re
...
!	REAL(8),PARAMETER :: s8 = 1.d0/tau
    ...
    REAL(8) :: tmp, Ri, Fx, Fy, Rj
    logical :: IsSolid(Nx,Ny)
...
SUBROUTINE Initialize_distributions
  ...
  ! —————————————————————————————————————————
  ! 1) Build the two solid walls and a capillary opening
  !—————————————————————————————————————————
  DO iY = 0, Ny+1
    DO iX = 0, Nx+1

      !  a) Side walls, everywhere outside the capillary opening
      !     If inside the rectangle y ≤ Y0+1 and |x−X0| > R → solid
      IF (iY <= Y0+1 .AND. ABS(iX - X0) > R) THEN
        IsSolid(iX,iY) = .TRUE.
      END IF

      !  b) The rest of the domain (iY > Y0+1) stays fluid/gas
      !     (so top is open automatically)
    END DO
  END DO

    ! —————————————————————————————————————————
  ! 2) Seed a short column of heavy fluid inside that capillary
  !—————————————————————————————————————————
  !    We'll fill the two rows just above the base (iY=0) inside the radius with C=1
  DO iY = 1, 2
    DO iX = X0 - R , X0 + R
      IF ((iX - X0)**2 + (iY - 0  )**2 <= R**2) THEN
        C(iX,iY) = C_L    ! heavy phase (liquid)
      END IF
    END DO
  END DO

  ! everything else is already C=0 (gas)
...
!
    ...
    !	CALL Calculate_Viscous_Force( tau_H, tau_L, DcDx(X,Y), DcDy(X,Y), gneq(:), C(X,Y), FmX, FmY )
        ...
        !Fx = mu(X,Y) * DcDx(X,Y) 
        Fx = mu(X,Y) * DcDx(X,Y) + FpX + FbX + FmX ! 
        Fy = mu(X,Y) * DcDy(X,Y) + FpY + FbY + FmY ! 
        !Fy = mu(X,Y) * DcDy(X,Y) 
...
SUBROUTINE Boundary_Conditions_f(f)
  USE SHARE, ONLY: Nx, Ny, IsSolid, ex, ey
  IMPLICIT NONE
  REAL(8), INTENT(INOUT) :: f(0:8, 0:Nx+1, 0:Ny+1)
  INTEGER, PARAMETER :: opp(0:8) = [0,3,4,1,2,7,8,5,6]
  INTEGER :: X, Y, dir, xN, yN

  ! 1) Periodic in x:
  f(:,   0, :) = f(:,   Nx, :)    ! left boundary copies from x = Nx
  f(:, Nx+1, :) = f(:,    1, :)    ! right boundary copies from x = 1

  ! 2) Open (zero-gradient) in y:
  f(:,   :, 0   ) = f(:,   :, 1   )    ! bottom (= 0) copies from y = 1
  f(:,   :, Ny+1) = f(:,   :, Ny )    ! top (= Ny+1) copies from y = Ny

  ! 3) Bounce-back on solid cells:
  DO Y = 1, Ny
    DO X = 1, Nx
      IF (IsSolid(X,Y)) THEN
        ! For each incoming direction dir into the solid
        DO dir = 1, 8
          xN = X + ex(dir)
          yN = Y + ey(dir)
          ! Only bounce back if neighbor is fluid
          IF (xN>=1 .AND. xN<=Nx .AND. yN>=1 .AND. yN<=Ny) THEN
            IF (.NOT. IsSolid(xN, yN)) THEN
              f(dir, X, Y) = f(opp(dir), xN, yN)
            END IF
          END IF
        END DO
      END IF
