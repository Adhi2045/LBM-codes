...
MODULE SHARE
    ...
    REAL(8),PARAMETER :: R = 16 !L0/8.d0 ! original
...
SUBROUTINE Initialize_distributions
    ...
    !    IF (Y == 1 .AND. X>=X0-15 .AND. X<=X0+17) THEN
    !        Rx = ABS(X - X0)
    !        C(X,Y) = 0.5d0 - 0.5d0 * TANH( (Rx - 16.d0)/W )
    !    END IF

    !    IF (Y == 0 .AND. X>=X0-15 .AND. X<=X0+17) THEN
    !        IsInlet(X,Y) = .TRUE.
    !        C(X,Y) = C(X,1)
    !    END IF
...
! ******************* solid boundary condition bounce back *****************************
 DO X = 1, Nx
    IF (ABS(X - X0) <= R) THEN
        Uy(X,Ny) = umax * (1.d0 - ((X - X0)**2.d0) / (R**2.d0))

    !    f(2, X, 0) = f(4, X, 0)
    !    f(5, X, 0) = f(7, X, 0) + 2.d0 * Wa(5) * Rho(X,1) * Uy(X,1) * ex(5)
    !    f(6, X, 0) = f(8, X, 0) + 2.d0 * Wa(6) * Rho(X,1) * Uy(X,1) * ex(6)


        f(2,X,0) = f(4,X,1) + (2.d0/3.d0) * Rho(X,1) * Uy(X,1)
        f(5,X,0) = f(7,X,1) - (1.d0/2.d0) * (f(1,X,1) - f(3,X,1)) + (1.d0/6.d0) * Rho(X,1) * Uy(X,1)
        f(6,X,0) = f(8,X,1) + (1.d0/2.d0) * (f(1,X,1) - f(3,X,1)) + (1.d0/6.d0) * Rho(X,1) * Uy(X,1)
    ...
    ! Top wall (iY = Ny+1)
  f(4, X, Ny+1) = f(2, X, Ny)
  f(7, X, Ny+1) = f(5, X, Ny) + (1.d0/2.d0) * (f(1,X,Ny) - f(3,X,Ny))
  f(8, X, Ny+1) = f(6, X, Ny) - (1.d0/2.d0) * (f(1,X,Ny) - f(3,X,Ny))
  Uy(X, Ny) = 0.0d0
  ...
  IF (ABS(X - X0) <= R) THEN
    C(X, 0) = 0.d0
  ELSE
    C(X, 0) = C(X, 1)   ! zero-gradient (to maintain interface continuity)
  END IF
END DO
! ******************* solid boundary condition bounce back *****************************
END
...
!    CALL Macroscopic_Properties_h
    ...
    DO X = 1, Nx
        IF (IsInlet(X,0)) THEN
            Uy(X,1) = Uyw  ! only force near inlet
            Ux(X,1) = 0.0d0
        END IF
    END DO
