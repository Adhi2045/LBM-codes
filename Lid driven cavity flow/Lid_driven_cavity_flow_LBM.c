...
int main(){
    ...
    for (ts=1; ts<=time; ts++){
        ...
        for (i=0; i<NX; i++){
            ...
            ft[4][i][j] = ft[2][i][j];
            ft[7][i][j] = ft[5][i][j] - (1.0f/12.0f) * rho * U_wall; //what is the value in this case?
            ft[8][i][j] = ft[6][i][j] + (1.0f/12.0f) * rho * U_wall;
        }

        for (j=0; j<NY; j++){

            i = 0; //Left wall stationary
            ft[1][i][j] = ft[3][i][j];
            ft[5][i][j] = ft[7][i][j];
            ft[8][i][j] = ft[6][i][j];

            i = NX-1; //Right wall stationary
            ft[3][i][j] = ft[1][i][j];
            ft[7][i][j] = ft[5][i][j];
            ft[6][i][j] = ft[8][i][j];        
        }
    ...
    out = fopen("ux_prof_ldc.dat","w");
    for (i=0; i<NX; i++){
        for (j=0; j<NY; j++){
            rho = 0.0; ux = 0.0; uy = 0.0;
            if (i,j!=0 && j!=NY-1 && i!=NX-1){
                for (a=0; a<9; a++){
                    rho += f[a][i][j];
                    ux += f[a][i][j]*ex[a];
                    uy += f[a][i][j]*ey[a];
